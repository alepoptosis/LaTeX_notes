\documentclass[12pt, letterpaper]{article} % defines the type (class) of the document, takes [extra parameters, separated by commas] like font (def = 10) and paper size (a3paper, a4paper) (e.g. [12pt, letterpaper])
...
\author{frostedmist\thanks{so done with the world}, too much tea} % author of the document + affiliations
...
% \newpage is an alt that breaks the page exactly at that point + starts a new column (if using more than one) - \clearpage is usually preferred
...
\clearpage
\subsection*{Sections hierarchy (commands):} % package titlesec allows customisation of sections
...
\clearpage
\section{Figures and referencing} \label{sec:fig} % labels the section for referencing
...
\begin{table}[h] % inserts table [h]ere
	...
	\begin{tabular}{|| l | c | r ||} % defines layout, number/align of columns & vertical lines
		...
		A	 &	23	 &	10000	\\ \hline
		B	 &	2	 &	66		\\ \hline
		C	 &	5	 &	58 		\\ [1ex] \hline % [ex] adds vertical space to row
	\end{tabular}
